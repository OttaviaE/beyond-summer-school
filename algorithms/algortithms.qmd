...
ggplot(long_dist, 
       aes(x = factor(name), y = value, color = name)) + geom_boxplot(width=.5) + theme_bw() +ylab("Symmetric Distance") + 
  scale_color_manual(values = wes_palette("AsteroidCity1"), 
...
1. Distance from $\text{TIF}^*$: $\Delta_x = |\text{TIF}_B - \text{TIF}_{x}|$, with $x \in \{B', \text{Frank}, \text{Léon}\}$
...
Boh mi verrà qualcosa in mente 
...
::: {.callout-tip}
## La scienza è uno sforzo *collettivo*
